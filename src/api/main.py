...
# Feature names the model expects (after ColumnTransformer)
...
def _predict_from_df(df: pd.DataFrame) -> dict:
    ...
    # Ensure column order matches training (ColumnTransformer handles missing
    # columns via imputation, but we warn on gross mismatches.)
    missing = set(X.columns) - set(EXPECTED_FEATURES)
    extra = set(EXPECTED_FEATURES) - set(X.columns)
    if missing or extra:
        # This should not happen if build_features is shared correctly,
        # but we surface it rather than silently failing.
        pass  # ColumnTransformer will handle via imputation / OHE

    pred = float(MODEL.predict(X)[0])
    hw = INTERVAL["half_width"]

    # Feature importance for this specific prediction (approximate via
    # permutation-style local importance — fast and interpretable.)
    base_pred = pred
    importances = []
    for col in X.columns:
        if col in ["season", "intensity"]:
            continue  # skip categorical for simple permutation
        X_pert = X.copy()
