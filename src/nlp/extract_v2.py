...
_PATTERNS = {
    ...
    "stocking_count": [
        r"(\d{3,6})\s*(?:fish|fingerlings|tilapia|stocked|seeds|juveniles)",
        r"stocked\s*(\d{3,6})",
        r"stocking\s*(?:count|density)?\s*(?:of|is)?\s*(\d{3,6})",
        r"(\d{3,6})\s*(?:fingerlings|fish|seeds)\s*(?:stocked|in|into)",
    ],
    "initial_weight_g": [
        r"initial\s*weight\s*(?:of|is)?\s*(\d+(?:\.\d+)?)\s*(?:g|grams|gram)",
        r"(\d+(?:\.\d+)?)\s*(?:g|grams|gram)\s*(?:fingerlings|seeds|fish|each|avg|average)",
        r"(?:fingerlings|seeds|fish)\s*(?:at|of|weighing)\s*(\d+(?:\.\d+)?)\s*(?:g|grams|gram)",
        r"(?:weight|size)\s*(?:of|is)?\s*(\d+(?:\.\d+)?)\s*(?:g|grams|gram)",
    ],
    "culture_days": [
        r"(\d{2,3})\s*(?:days|day)",
        r"culture\s*(?:period|duration|days)?\s*(?:of|is)?\s*(\d{2,3})",
        r"grow\s*(?:them|the fish)?\s*for\s*(\d{2,3})\s*(?:days|day)",
        r"(?:planning|plan|going)\s*(?:to|for)\s*(\d{2,3})\s*(?:days|day)",
    ],
    "mean_temperature_c": [
        r"(?:temperature|temp)\s*(?:is|of|around|about|avg|average)?\s*(\d{2}(?:\.\d+)?)\s*(?:°?C|degrees?)",
        r"(\d{2}(?:\.\d+)?)\s*(?:°?C|degrees?)\s*(?:temperature|temp|average|avg)",
        r"temp\s*(?:around|about|is)?\s*(\d{2}(?:\.\d+)?)",
    ],
    "max_temp_c": [
        r"max(?:imum)?\s*(?:temperature|temp)\s*(?:is|of|around)?\s*(\d{2}(?:\.\d+)?)\s*(?:°?C|degrees?)",
        r"(?:temperature|temp)\s*(?:goes|reaches|peaks|high|up)\s*(?:up\s*to|at|to)\s*(\d{2}(?:\.\d+)?)",
        r"(?:hot|warm)\s*(?:up\s*to|to|at)?\s*(\d{2}(?:\.\d+)?)",
    ],
    "min_temp_c": [
        r"min(?:imum)?\s*(?:temperature|temp)\s*(?:is|of|around)?\s*(\d{2}(?:\.\d+)?)\s*(?:°?C|degrees?)",
        r"(?:temperature|temp)\s*(?:drops\s*to|goes\s*down\s*to|low|down)\s*(?:to)?\s*(\d{2}(?:\.\d+)?)",
        r"(?:cold|cool)\s*(?:down\s*to|to|at)?\s*(\d{2}(?:\.\d+)?)",
    ],
    "mean_do_mg_l": [
        r"(?:do|dissolved\s*oxygen|oxygen)\s*(?:is|of|around|avg|average|level|concentration)?\s*(\d+(?:\.\d+)?)\s*(?:mg/?l|mg\s*per\s*litre|mg\s*per\s*liter|mg/l)?",
        r"(?:do|dissolved\s*oxygen|oxygen)\s*(?:level|concentration)?\s*(?:is|of|around)?\s*(\d+(?:\.\d+)?)",
        r"(?:oxygen|do)\s*(?:level|concentration)?\s*(?:is|of|around)?\s*(\d+(?:\.\d+)?)",
    ],
    "min_do_mg_l": [
        r"(?:lowest|minimum|min)\s*(?:do|dissolved\s*oxygen|oxygen)\s*(?:is|of|drops\s*to|goes\s*down\s*to)?\s*(\d+(?:\.\d+)?)",
        r"(?:do|dissolved\s*oxygen|oxygen)\s*(?:drops|falls|goes\s*down)\s*(?:to|as\s*low\s*as)?\s*(\d+(?:\.\d+)?)",
    ],
    "mean_ph": [
        r"(?:ph|pH)\s*(?:is|of|around|avg|average)?\s*(\d+(?:\.\d+)?)",
        r"(?:ph|pH)\s*(?:level|value)?\s*(?:is|of|around)?\s*(\d+(?:\.\d+)?)",
    ],
    "min_ph": [
        r"(?:lowest|minimum|min)\s*(?:ph|pH)\s*(?:is|of|drops\s*to|goes\s*down\s*to)?\s*(\d+(?:\.\d+)?)",
        r"(?:ph|pH)\s*(?:drops|falls|goes\s*down)\s*(?:to|as\s*low\s*as)?\s*(\d+(?:\.\d+)?)",
    ],
    "feed_protein_pct": [
        r"(?:protein|feed\s*protein)\s*(?:is|of|around)?\s*(\d{2})\s*%",
        r"(\d{2})\s*%\s*(?:protein|feed\s*protein)",
        r"(?:protein|feed)\s*(?:content|level)?\s*(?:is|of|around)?\s*(\d{2})\s*%",
    ],
    "season": [
